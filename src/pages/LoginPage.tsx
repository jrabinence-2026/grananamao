...
// 2. FUNCIONALIDADE: Aplica máscara de telefone em tempo real: (00) 00000-0000 ou (00) 0000-0000
...
// 5c. FUNCIONALIDADE: Valida se o telefone tem 10 ou 11 dígitos
...
export function LoginPage() {
  const { login, signUp, isAuthed, loading, authError, loginWithGoogle, user } = useStore();
  ...
  const forgotName = `${forgotFirstName} ${forgotLastName}`.trim();
  ...
  // 8b. FUNCIONALIDADE: Valida formatos do telefone e e-mail
  const isCpfFormatOk = validatePhone(cpf);
  ...
  useEffect(() => {
    if (isAuthed && user) {
      if (user.phone && user.phone.startsWith("000000000")) {
        // Define flag para exibir mensagem de boas-vindas na tela de perfil
        sessionStorage.setItem("grananamao.welcome_new_user", "true");
        navigate({ to: "/home", search: { tab: "profile" } });
      } else {
        navigate({ to: "/home", search: { tab: "home" } });
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      const { data: available } = await supabase
        .rpc("is_phone_available", { check_phone: cpf });
  ...
  const switchMode = (m: "login" | "signup" | "forgot_password") => {
    ...
    setFirstName(""); setLastName(""); setUsername(""); setCpf(""); setBirthDate("");
    ...
    setForgotStep(1); setForgotFirstName(""); setForgotLastName(""); setForgotIdentifier("");
  ...
  const executeSignUp = async () => {
    ...
    await signUp(signupEmail, signupPwd, {
      ...
      phone: cpf.replace(/\D/g, ""),   // Remove formatação antes de salvar
      birthDate,
    });
  };
  ...
  const handleVerifyForgotDetails = async (e: React.FormEvent) => {
    ...
    try {
      const { data: verified, error } = await supabase.rpc("verify_reset_details", {
        check_phone: forgotCpf.replace(/\D/g, ""),
      ...
      if (verified) {
        ...
        setForgotErrorMsg("⚠️ Você não acertou o mínimo de perguntas para alterar a senha. Tente entrar em contato com o suporte.");
      }
    } catch (err: any) {
      setForgotErrorMsg("⚠️ Você não acertou o mínimo de perguntas para alterar a senha. Tente entrar em contato com o suporte.");
    } finally {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center px-6 py-10 bg-navy">
      <div className="w-full max-w-sm mx-auto flex flex-col items-center">
        ...
        />
        <h1 className="text-3xl font-extrabold text-cream tracking-tight">GranaNaMao</h1>
        ...
        <div className="grid grid-cols-2 w-full mt-6 p-1 rounded-full bg-navy-elevated border border-cream/10">
          {(["login", "signup"] as const).map((m) => (
            <button
              ...
              className={`py-2.5 rounded-full text-sm font-semibold transition-colors ${mode === m ? "bg-orange text-white" : "text-cream-muted hover:text-cream"
                }`}
            >
        ...
        {mode === "login" && (
          <form onSubmit={handleLogin} className="w-full mt-6 space-y-3.5">
            ...
            {/* Campo: Celular, nome de usuário ou e-mail */}
            <input
              value={identifier}
              onChange={(e) => setIdentifier(e.target.value)}
              placeholder="Celular, usuário ou e-mail"
            ...
            {authError && (
              <p className={`text-xs text-center font-medium ${authError.startsWith("✅") ? "text-[#4CAF50]"
            ...
            <div className="flex items-center gap-3 py-1">
              ...
              <span className="text-[10px] text-cream-muted/40 uppercase font-bold tracking-wider">ou</span>
              <div className="h-[1px] flex-1 bg-cream/10"></div>
            </div>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl border border-cream/10 bg-navy-elevated text-cream font-semibold hover:bg-cream/5 transition-all flex items-center justify-center gap-2 text-sm active:scale-[0.98]"
            >
            ...
            <div className="text-center pt-1.5">
              <button
                ...
                className="w-full py-3 rounded-xl border border-orange/50 bg-transparent text-xs text-orange hover:bg-orange/10 transition-all font-semibold"
              >
                Esqueci minha senha
              </button>
            </div>
        ...
        {mode === "signup" && (
          <form onSubmit={handleSignUp} className="w-full mt-6 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  autoComplete="username"
                  maxLength={20}
                  className={`w-full pl-9 pr-10 py-3.5 rounded-xl bg-navy-elevated border text-cream placeholder-cream-muted/50 outline-none transition-colors text-sm ${usernameStatus === "available" ? "border-[#4CAF50]/60 focus:border-[#4CAF50]" :
            ...
            {/* Campo: Celular com máscara automática e verificação em tempo real */}
            <div className="space-y-1.5">
              <div className="relative">
                <input
                  value={cpf}
                  onChange={(e) => setCpf(maskPhone(e.target.value))}
                  placeholder="Celular (00) 99999-9999"
                  inputMode="numeric"
                  maxLength={15}
                  className={`w-full pr-10 pl-4 py-3.5 rounded-xl bg-navy-elevated border text-cream placeholder-cream-muted/50 outline-none transition-colors text-sm ${cpfStatus === "available" ? "border-[#4CAF50]/60 focus:border-[#4CAF50]" :
                      cpfStatus === "taken" || (cpf.replace(/\D/g, "").length >= 10 && !isCpfFormatOk) ? "border-orange/60 focus:border-orange" :
                        "border-cream/10 focus:border-orange"
                    }`}
                />
                <div className="absolute right-4 top-1/2 -translate-y-1/2">
                  ...
                  {(cpfStatus === "taken" || (cpf.replace(/\D/g, "").length >= 10 && !isCpfFormatOk)) && <XCircle size={15} className="text-orange" />}
                </div>
              </div>
              {cpf.replace(/\D/g, "").length >= 10 && !isCpfFormatOk && (
                <p className="text-[11px] text-orange pl-1">⚠️ Celular inválido.</p>
              )}
              {cpfStatus === "taken" && (
                <p className="text-[11px] text-orange pl-1">⚠️ Este celular já está cadastrado.</p>
              )}
            </div>
            ...
            <div className="space-y-1.5 w-full">
              ...
              <input
                ...
                className={`w-full h-[52px] px-4 py-3 rounded-xl bg-navy-elevated border text-cream outline-none transition-colors text-sm ${birthDate && isAgeOk ? "border-[#4CAF50]/60 focus:border-[#4CAF50]" :
            ...
            <div className="space-y-1.5">
              <div className="relative">
                <input
                  ...
                  placeholder="E-mail"
                  autoComplete="email"
                  className={`w-full pr-10 pl-4 py-3.5 rounded-xl bg-navy-elevated border text-cream placeholder-cream-muted/50 outline-none transition-colors text-sm ${emailStatus === "available" ? "border-[#4CAF50]/60 focus:border-[#4CAF50]" :
          ...
          <div className="w-full mt-6 space-y-4">
            ...
            {forgotStep === 1 && (
              ...
              >
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      ...
                      onChange={(e) => setForgotFirstName(capitalizeName(e.target.value))}
                      placeholder="Nome"
                      required
                      className="w-full px-4 py-3.5 rounded-xl bg-navy-elevated border border-cream/10 text-cream placeholder-cream-muted/50 outline-none focus:border-orange transition-colors text-sm"
                    />
                    <input
                      ...
                      onChange={(e) => setForgotLastName(capitalizeName(e.target.value))}
                      placeholder="Sobrenome"
                      required
                      className="w-full px-4 py-3.5 rounded-xl bg-navy-elevated border border-cream/10 text-cream placeholder-cream-muted/50 outline-none focus:border-orange transition-colors text-sm"
                    />
                ...
                <button
                  type="submit"
                  disabled={!forgotFirstName.trim() || !forgotLastName.trim() || !forgotIdentifier.trim()}
            ...
            {/* ETAPA 2: Celular e Data de Nascimento */}
            {forgotStep === 2 && (
              <form onSubmit={handleVerifyForgotDetails} className="space-y-3.5">
                <div className="space-y-1">
                  ...
                  <input
                    value={forgotCpf}
                    onChange={(e) => setForgotCpf(maskPhone(e.target.value))}
                ...
                <div className="space-y-1 w-full">
                  ...
                  <input
                    ...
                    className="w-full h-[52px] px-4 py-3 rounded-xl bg-navy-elevated border border-cream/10 text-cream outline-none focus:border-orange transition-colors text-sm"
                  />
                </div>
            ...
            <div className="text-center pt-1.5 space-y-3">
              ...
              <a
                href="https://wa.me/5598984026886?text=preciso%20de%20ajuda%20para%20redefinir%20ou%20acessar%20minha%20senha"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-block text-[11px] text-cream-muted hover:text-cream transition-colors font-medium underline"
              >
                Precisa de ajuda para acessar a conta? Fale com o suporte
              </a>
            </div>
